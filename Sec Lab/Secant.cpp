...
*/
        ...
        cout << "\t\t\t" << x1 << "\t\t\t" << x2 << "\t\t\t" << xm1 << "\t\t\t" << xm2 << endl;

                                                                                              
    } while (fabs(xm2 - xm1) >= e); /* check if the error is greater than the
desired accuracy. fabs function is used to return non-negative float value of
its argument */
    return xm1;
}
double f(double x)
{
    /* write the equation whose roots are to be determined. Here we are using
equation as x^3 + x - 1 */
    double res = pow(x, 2.2) - 69;
    return res;
}